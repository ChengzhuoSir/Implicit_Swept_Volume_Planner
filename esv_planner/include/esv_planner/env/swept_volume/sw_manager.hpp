...
#include <Eigen/Eigen>
#include "esv_planner/optimization/svsdf_backend/utils/trajectory.hpp"
#include "esv_planner/optimization/svsdf_backend/utils/Shape.hpp"
...
#include <esv_planner/env/swept_volume/sw_calculate.hpp>
...
constexpr double kSweptPi = 3.1415926536;
...
public:
    ...
    inline void initShape(Config &conf)
    {
        ...
        {
            current_robot_shape = new MeshIglShape(conf);
            std::cout << "======= MeshIglShape =======" << std::endl;
        }
    }
    ...
    {
        return ix >= 0 && ix < map_Xsize && iy >= 0 && iy < map_Ysize && iz >= 0 && iz < map_Zsize;
    }
    bool isIndexOccupied(int ix, int iy, int iz)
    {
        if (!isIndexValid(ix, iy, iz))
        {
            return true;
        }
        const int bytes_len_of_last_dim = (map_Ysize + 7) / 8;
        const int byte_idx = ix * bytes_len_of_last_dim + (iy / 8);
        const int bit_idx = iy % 8;
        return (map_kernel[byte_idx] & PCSmapManager::or_mask[bit_idx]) != 0;
    }
    ...
    {
        int father_i = int(kernel_count * ((father_yaw + kSweptPi) / (2 * kSweptPi)));
        int ret_i = father_i;
        bool res = false; 
        if (visit_kernels_by_distance(ret_i, father_i, ind))
        {
            child_yaw = 2 * kSweptPi * (ret_i) / kernel_count - kSweptPi;
            return true;
        }
        return false;
    }
