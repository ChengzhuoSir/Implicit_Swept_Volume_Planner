...
public:
  ...
  void init(const GridMap& map, const CollisionChecker& checker,
            ...
            int num_samples, int knn, int max_paths, double inscribed_radius);
  ...
  int startDegree() const { return adjacency_.empty() ? 0 : static_cast<int>(adjacency_[0].size()); }
  int goalDegree() const { return adjacency_.size() < 2 ? 0 : static_cast<int>(adjacency_[1].size()); }
private:
