...
#include <esv_planner/optimization/svsdf_backend/utils/Visualization.hpp>
#include <esv_planner/optimization/svsdf_backend/utils/quickhull.hpp>
#include <esv_planner/optimization/svsdf_backend/utils/geo_utils.hpp>
...
inline std::string ResolveShapePath(const Config &config)
{
    if (!config.inputdata.empty() && config.inputdata[0] == '/')
    {
        return config.inputdata;
    }
    const std::string package_root = ros::package::getPath("esv_planner");
    if (package_root.empty())
    {
        return config.inputdata;
    }
    return package_root + "/" + config.inputdata;
}
...
namespace shape
{
    ...
    private:
        ...
        NumSDFGridCell *num_sdf_map{nullptr};
    ...
    public:
        ...
        virtual double getonlySDF(const Eigen::RowVector3d &pos_rel) { return getonlySDF_igl(pos_rel); };
        virtual double getonlySDF(const Eigen::RowVector3d &pos_rel, const Eigen::Matrix3d &R_obj)
        {
            return getonlySDF_igl(pos_rel * R_obj);
        };
        ...
        BasicShape(const Config &conf) : config(conf)
        {
            ...
            string input = ResolveShapePath(config);
    ...
    class MeshIglShape : public BasicShape
    {
    public:
        explicit MeshIglShape(const Config &conf) : BasicShape(conf)
        {
