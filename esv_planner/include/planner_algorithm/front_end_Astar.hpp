...
{ 
    public:
        ...
        int total_kernel  = 0;
        int kernel_size   = 3;
        int max_expansions = 150000;
        double max_search_time = 1.5;
...
inline void AstarPathSearcher::AstarPathSearch(Vector3d start, Vector3d end)
{
    ...
    Vector3d start_pos = start;
    Vector3d end_pos = end;
    start_pos(2) = 0.0;
    end_pos(2) = 0.0;
    if( !(environment -> occupancy_map) -> isInMap(start_pos) || !(environment -> occupancy_map) -> isInMap(end_pos) )
    {
       ROS_ERROR("[A*] start or target position is out of map. start=(%.3f, %.3f, %.3f) goal=(%.3f, %.3f, %.3f) bounds=[(%.3f, %.3f, %.3f) -> (%.3f, %.3f, %.3f)]",
                 start_pos(0), start_pos(1), start_pos(2),
                 end_pos(0), end_pos(1), end_pos(2),
                 environment->occupancy_map->boundary_xyzmin(0),
                 environment->occupancy_map->boundary_xyzmin(1),
                 environment->occupancy_map->boundary_xyzmin(2),
                 environment->occupancy_map->boundary_xyzmax(0),
                 environment->occupancy_map->boundary_xyzmax(1),
                 environment->occupancy_map->boundary_xyzmax(2));
       success_flag = false;
       return ;
    }
    //index of start_point and end_point
    Vector3i start_idx = (environment -> occupancy_map) -> getGridIndex(start_pos);
    Vector3i end_idx   = (environment -> occupancy_map) -> getGridIndex(end_pos);
    ...
    while ( !openSet.empty() ){
        ...
        if (expansions > max_expansions ||
            (ros::Time::now() - time_1).toSec() > max_search_time) {
            success_flag = false;
            ROS_WARN("[A*] search aborted: expansions=%d limit=%d time=%.3f limit=%.3f",
                     expansions, max_expansions, (ros::Time::now() - time_1).toSec(),
                     max_search_time);
            return;
        }
