...
int main(int argc, char** argv) {
  ros::init(argc, argv, "test_optimizer_r2_continuous_polynomial",
  ...
  for (size_t i = 0; i < segments.size(); ++i) {
    ...
    if (seg.risk != RiskLevel::LOW) {
      continue;
    }
    const double chain_clearance =
        continuousChainClearance(seg.waypoints, svsdf, 0.05);
    if (chain_clearance + 1e-6 < 0.1) {
      continue;
    }
    target_seg_idx = i;
    target_waypoints = seg.waypoints;
    target_chain_clearance = chain_clearance;
    break;
  }

  if (target_waypoints.empty()) {
    std::cerr << "[test] FAIL: expected a fixed-case LOW segment above safety margin\n";
    return 1;
  }
  ...
  const OptimizerResult result =
      optimizer.optimizeR2Detailed(target_waypoints, seg_time);
  const double clearance =
      result.traj.empty() ? -kInf : svsdf.evaluateTrajectory(result.traj);

  std::cout << "[test] target_low_seg=" << target_seg_idx
            << " wps=" << target_waypoints.size()
            << " chain_clearance=" << target_chain_clearance
            << " r2_success=" << (result.success ? 1 : 0)
            << " r2_clearance=" << clearance
            << " source_mode=" << static_cast<int>(result.source_info.source_mode)
            << " used_guard=" << (result.source_info.used_guard ? 1 : 0)
            << " has_higher_order=" << (hasHigherOrderPositionTerms(result.traj) ? 1 : 0)
            << "\n";

  if (!result.success || result.traj.empty()) {
    std::cerr << "[test] FAIL: expected R2 optimizer to produce a trajectory\n";
    return 1;
  }
  if (clearance + 1e-6 < params.safety_margin) {
    std::cerr << "[test] FAIL: expected R2 trajectory to satisfy safety margin\n";
    return 1;
  }
  if (result.source_info.used_guard) {
    std::cerr << "[test] FAIL: expected LOW segment to stay on continuous source path\n";
    return 1;
  }
  if (!hasHigherOrderPositionTerms(result.traj)) {
    std::cerr << "[test] FAIL: expected continuous R2 trajectory to use higher-order position terms\n";
    return 1;
  }
