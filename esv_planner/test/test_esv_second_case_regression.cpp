...
SecondCaseReport runSecondCase() {
  ...
  const SE2State start(0.72, 5.31, 0.29);
  const SE2State goal(9.14, 5.74, 1.64);
...
int main(int argc, char** argv) {
  ros::init(argc, argv, "test_esv_second_case_regression",
            ros::init_options::NoSigintHandler);
  ros::Time::init();

  const SecondCaseReport report = runSecondCase();
  ...
  if (report.topo_paths == 0) {
    std::cerr << "[test] FAIL: topology stage produced no paths for the second case\n";
    return 1;
  }

  // Baseline harness only: record real metrics for the second case without
  // forcing full acceptance yet. Later rebuild stages will tighten this into a
  // hard gate once the strict paper-first pipeline is in place.
  return 0;
}
