...
#include <isweep_planner/optimization/svsdf_backend/utils/lmbm.h>
#include <isweep_planner/optimization/svsdf_backend/utils/Visualization.hpp>
...
public:
 ...
 static inline double costFunctionLmbmParallel(void *ptr,
                                          ...
                                          const int n)
    {
        ...
        // Clamp gradient magnitude and sanitize NaN components
        {
            double grad_sq = 0.0;
            for (int gi = 0; gi < n; ++gi) {
                if (!std::isfinite(g[gi])) {
                    g[gi] = 0.0;
                }
                grad_sq += g[gi] * g[gi];
            }
            ...
            if (grad_norm > 1e4) {
                const double scale = 1e4 / grad_norm;
                for (int gi = 0; gi < n; ++gi) {
                    g[gi] *= scale;
                }
            }
...
public:
                            ...
                            double &costp)
    {
        ...
        for (int i = 0; i < ob_pts.size(); i++) 
        {
            ...
            sdf_cost = -1.0;
            smoothedL1(turcation - sdf_value, 0.05, sdf_cost, grad_out );
                                ...
                                double &costp)
    {
        ...
        smoothedL1(safety_hor - sdf_value ,0.05, sdf_cost ,sdf_out_grad);
        // Regularize St before inversion: clamp diagonal away from zero
        Eigen::Matrix3d St_reg = St;
        for (int d = 0; d < 3; ++d) {
            if (std::abs(St_reg(d, d)) < 1e-6) {
                St_reg(d, d) = (St_reg(d, d) >= 0.0) ? 1e-6 : -1e-6;
            }
        }
        Eigen::Vector3d sdf_grad = -sdf_out_grad * (-(St_reg.inverse()).transpose()* rotate * gradp_rel);
